...
- [x] **R1変更**: 10Ω → 100Ω（ノイズフロア 1.28A → 0.16A に改善、クリップ~23A）
- [x] **実電線クランプで精度確認**: 10〜11A をクランプメーターと比較 → 一致（10.64A）
- [ ] **C1交換**: 47μF電解 → 10μF + 100nF セラミック並列（安全性向上・任意）
- 詳細: `.steering/design.md` の「電流計測ノイズ対策」セクション参照

#### Step3-2追記: MCP3208入力保護（Step5後に再検討）
- [ ] **100A突入対策を検討・実装**（現状R1=100Ω、無保護）
  - 案A: R1=33Ω（部品追加なし、100Aまで自己制限、ノイズフロア~0.39A）
  - 案B: R1=100Ω + ショットキーダイオードクランプ + 1kΩ直列（ノイズフロア0.16A維持）
  - 参考: 100A突入時、測定ノードが -1.35V〜4.65V になりMCP3208の絶対最大定格(3.9V)を超える
