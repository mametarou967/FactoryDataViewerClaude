...
- **理由**: パトライトが回転しており、瞬時値だけでは光源が反対側を向いているタイミングで消灯誤判定が発生する。直近1.5秒間のmax値を保持することで確実に点灯を検出できる。Core1を専用に割り当てることでタイムアウト内（2500ms）に即応答可能。
- **電流も同様**: 交流のRMS計算にも高速サンプリングが必要なためCore1で担当。
- **コア間共有**: mutex_tで`patlite_max[3]`・`current_rms`・`sensor_error`・`core1_heartbeat`を保護。
...
## エッジのLED用途
- **決定**: D1=ハートビート(5秒周期100msパルス), D2=LoRa通信状態(受信時100ms点滅), D3=モード表示(黄・設定/テストモード中点灯), D4=センサー異常(赤・エラー時点灯)
- **理由**: ユニットは高所（3〜4m）に設置されるため、パネル引き出しLEDで遠目から一目で状態を把握できる構成にした。ハートビートでフリーズを検知、D2でLoRa断線を検知できる。
- **D1の実装**: Core0のみでなくCore1の生存も確認する。Core1がloop1()内でcore1_heartbeatカウンタをインクリメントし、Core0が5秒ごとにカウンタの変化を確認してD1を制御。Core1フリーズ時はカウンタが変わらずD1が止まる。メンテ中（黄LED点灯時）はD1点滅を抑制する。

## 多工場対応: チャンネルで工場を分離
- **決定**: 工場ごとに専用のGW（RPi5 + USB E220）を用意し、LoRaチャンネルで工場間を分離する。各工場のアドレス空間は独立（同じアドレス体系を工場ごとに使い回し可）。
- **理由**: 各工場が独立したGWを持つため、アドレスをグローバル管理する必要がない。チャンネル分離により同一建物内でも電波干渉なし。新工場追加時はチャンネルを1つ増やすだけでスケールする。
- **チャンネル割り当て**: config.yamlの `gw_channel` で工場ごとに設定。**工場番号とチャンネル番号を一致させる規則**（例: 工場2=CH2, 工場3=CH3, ...）。JP版 BW125kHz時: 920.6 + CH × 0.2 MHz。現在は工場2（CH2=921.0MHz）に着手中。

## LoRaアドレス体系: 0xMMTT形式
- **決定**: アドレスを `0xMMTT`（MM=機械番号、TT=ユニット種別）で構造化する。TT: 01=パトライト, 02=電流, 03=兼務, 04〜=将来拡張。
- **理由**: 連番では機械数・ユニット種類が増えたとき範囲を再設計する必要がある。0xMMTT形式ならアドレスから機械番号・種別を直読みでき、拡張時も既存アドレスに影響しない。255台・255種別まで対応可能。
