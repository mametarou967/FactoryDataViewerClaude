...
VCC(3.3V)
 ...
 R2(10k)
  ...
  ├──── [MCP3208 CH0] ←── 計測点
  │
[CTL-24-CLS 白線] ─ R1(10Ω) ─ [CTL-24-CLS 黒線]
                                    │
                                   GND
 R3(10k)
  │
...
- **R1=10Ω**: CTL-24-CLSのバーデン抵抗（データシート仕様値）
...
**K=1近似（Io ≥ 10A）**:
```
Eo_rms = Io × 10 / 2000 = Io / 200  (Vrms)
→ Io = Eo_rms × 200  (Arms)
```

K係数の実態（RL=10Ω, データシートK特性グラフより）:
| 電流 Io | K値 | 備考 |
|--------|-----|------|
| 1A | ≈0.75 | 低電流域・誤差大 |
| 10A | ≈0.95 | 実用範囲内 |
...
|------|--------|----------------|------|
| 3A | 15mV | ~19カウント | 稼働判定閾値付近 |
| 10A | 50mV | ~62カウント | |
| 50A | 250mV | ~310カウント | 工場機械典型値 |
| 100A | 500mV | ~620カウント | |
| 300A | 1500mV | ~1861カウント | 定格（CH0: 0〜3.77V → Zenerクランプ動作） |
...
**対策（部品到着後に実施）**:
1. C1 交換: 47μF 電解 → **10μF + 100nF セラミック並列**
   - 安全性向上（電解コン廃止）
   - DCバイアス: τ = 5kΩ × 10μF = 50ms >> 20ms(50Hz) ✅
   - 4kHz EMIシャント: Z = 4Ω ✅
2. VREF 安定化: MCP3208 VREF(pin15)–AGND(pin14) 間に **10μF + 100nF セラミック並列**を追加
   - VREF を VCC 変動から切り離す
   - DC バイアスと VREF が両方安定 → 比率が一定 → ノイズ激減
