...
- **R1=100Ω**: CTL-24-CLSのバーデン抵抗（ノイズフロア改善・クリップ~23A）
...
|------|--------|----------------|------|
| 1A | 5mV | ~6カウント | ノイズフロア付近 |
| 3A | 150mV | ~186カウント | 稼働判定閾値付近 |
| 10A | 500mV | ~620カウント | 実機確認済み（10.64A計測） |
| 22A | 1100mV | ~1365カウント | ADCクリップ直前（測定限界） |
| 23A↑ | — | ADC飽和 | 22A超の検知用途（クリップで対応） |
...
**実施した対策**:
1. **R1変更**: 10Ω → **100Ω**（主要改善）
   - ノイズフロア: ~1.15A → **~0.16A** に改善（約7倍）
   - クリップ電流: ~23A（22A超はADC飽和で検知）
   - 実機確認: 10〜11A をクランプメーターと比較 → 一致（10.64A）
