...
## GW設定ファイル構造（案）
```yaml
gw_channel: 2             # 工場ごとに異なるチャンネルを割り当て（電波干渉回避）
...
| 工場 | チャンネル | 周波数 | アドレス空間 |
|------|----------|--------|------------|
| 工場1 | 2 | 921.0 MHz (Zone A) | 0x0000〜0xFF03（工場内で完結） |
| 工場2 | 3 | 921.2 MHz (Zone A) | 0x0000〜0xFF03（工場内で完結） |
| 工場3 | 4 | 921.4 MHz (Zone A) | 0x0000〜0xFF03（工場内で完結） |
