...
- [x] D4センサー異常LED: TSL2561初期化失敗時に点灯
...
#### Step3-2補足: 電流計測ノイズ対策 ✅
- [x] **VREF安定化**: MCP3208 VREFピン(15番)とAGND(14番)間に 10μF + 100nF セラミック並列を追加
- [x] **R1変更**: 10Ω → **33Ω**（100Aピーク保護 + 0〜30A高精度化、誤差0.5A以内確認済み）
- [x] **実電線クランプで精度確認**: 誤差0.5A以内（許容範囲内）
- [x] **ノイズフロア補正（ソフト）**: CURRENT_NOISE_FLOOR_A定数 + sqrt(I²-N²)補正を実装
- [ ] **C1交換**: 47μF電解 → 10μF + 100nF セラミック並列（安全性向上・任意）
- 詳細: `spec/design.md` の「電流計測ノイズ対策」セクション参照

#### Step3-2追記: MCP3208入力保護 ✅
- [x] **100A突入対策**: R1=33Ωで100Aピークまで安全（ADC入力 1.65±1.49V, 3.3V以内）

### Step4: UI・設定
- [x] SSD1306表示ドライバ（Wire1使用）
- [x] ボタン入力処理（デバウンス含む）
- [x] 設定UI（液晶+3ボタンによるメニュー）: 設定確認・ADDH変更・CH変更
- [x] 設定確認画面（ConfView: ADDH/ADDL/CH/REG0/REG1/REG3表示）
- [x] E220設定の読み書き（設定値をE220のE2PROMに書込み・読出し）
- [ ] LoRa経由リモート設定
- [ ] USBシリアル経由設定

### Step5: ローカルテストモード ✅
- [x] パトライトユニット: 3センサーのlux値を液晶表示
- [x] 電流ユニット: AC波形スナップショット表示（計測指示→短期サンプリング→表示） + RMS電流値表示

## Phase 3 & 4: GW + Webアプリ統合（app.py 再実装）
※ lora_logger.py と app.py を統合した1アプリとして実装する

### Step1: 基盤・ポーリング ✅
- [x] GW設定ファイル設計・実装（YAML）
- [x] E220制御ドライバ（USB serial）
- [x] バックグラウンドポーリングスレッド実装（1分周期）
- [x] タイムアウト・再送処理
- [x] データ統合（patlite + current → 1レコード）
- [x] CSV書き込み（機械ごとサブディレクトリ）
- [x] スレッドセーフなコマンドキュー実装（ポーリングとメンテ操作の調停）※ Step3向け枠として実装済み

### Step2: 監視Web UI ✅
- [x] config.yaml準拠の台数に対応したデータ読み取り構造
- [x] 現在状態一覧画面（台数はconfig.yaml準拠、現在11台）
- [x] 状態判定ロジックを設定ファイルベースに変更
- [x] 日別・月別グラフ機能を多機械対応に拡張（URL: /machine/<name>/date/<date>/...）
- [x] 品目突合せ表示

### Step3: メンテナンスWeb UI ✅
- [x] メンテナンス画面（ユニット選択・操作ボタン）
